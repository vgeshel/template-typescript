...
**How:** Use fast-check arbitraries from the project's shared test directory. Search for existing arbitraries before creating new ones:

- Agent output arbitraries — content, formatting, and transport layers
- API response arbitraries — GitHub API response shapes
- File content arbitraries — markdown and state file shapes
- Command output arbitraries — git and other CLI output shapes
...
- Place in a dedicated component/integration test directory
- Mock only the agent SDK and external services
- Let the parsing pipeline run for real (JSON extraction, Zod validation)
- Write to real temp directories when testing file I/O
- Verify results on disk, not just function calls

**E2E tests:** New agent workflows should have end-to-end integration tests
that run with real agents under `INTEGRATION_TEST=true`.
...
### Duplicated Conditional Patterns

When copying error-handling or conditional logic from one function to another, you MUST also copy
(or write new) tests for BOTH branches of every conditional in the copied code.

Common scenario: a workflow has a bootstrap agent and a verification agent with identical error handling.
The bootstrap agent has tests for both branches of `errors.length > 0 ? errors.join() : fallback`.
When the same pattern is duplicated for the verification agent, tests for both branches must exist
for the verification agent too.

**Checklist when duplicating conditional code:**

1. Identify every conditional (`if`, `?:`, `??`, `||`) in the copied block
2. Find the tests covering both branches in the original context
3. Write equivalent tests for the new context
4. Run `bun test:coverage` and verify 100% — do NOT assume the tests are complete
...
1. Search the project for similar test files
2. Read existing test files for patterns
3. Copy the test structure for similar functionality
