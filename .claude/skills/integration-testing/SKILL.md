...
function createIntegrationMockFunctions(): WorkflowFunctions {
  return {
    ...
    // IMPORTANT: Every vi.fn() must use the real function's signature.
    // Read the source module to get the correct parameter and return types.
    ...
    addComment: vi
      .fn<
        (
          ...
          issueNumber: number,
          body: string,
        ) => ResultAsync<{ id: number }, GitHubError>
      >()
      .mockReturnValue(okAsync({ id: 1 })),
  }
}
```
