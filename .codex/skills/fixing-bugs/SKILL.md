...
```
Bug Fix Progress:
- [ ] Step 1: Understand the bug report / CI failure artifact
- [ ] Step 1.5: Reproduce the exact failing command/path from the artifact
...
- [ ] Step 6: Re-run the exact failing command/path and confirm pass
- [ ] Step 7: Run full test suite
- [ ] Step 8: Refactor if needed (tests must stay green)
```
...
## Step 1.5: Reproduce the Exact Failing Path (for CI/Test Failures)

When the bug report comes from CI or a failing test artifact:

1. Extract the exact failing command/test path from logs.
2. Run that exact command/path before code changes.
3. Confirm the observed failure matches the reported symptom.

If exact local reproduction is impossible, stop and report:

- what command/path you tried,
- why it could not run locally,
- and that status is `patched, not proven locally` until CI is green.
...
## Step 6: Re-run Exact Failing Command/Path

Before broad verification, rerun the exact command/path that failed originally:

```
bun test:run <exact-failing-test-file-or-command>
```

This is the proof gate for claiming `fixed`.
